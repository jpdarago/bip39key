...
<p>Compare them with <code>gpg --list-keys --with-subkey-fingerprint</code> after import.</p>"#
    ...
    let created_display = if receipt.created == 1231006505 {
        "2009-01-03T18:15:05Z (Bitcoin genesis block)".to_string()
    } else {
        receipt.created.to_string()
    };
    let expires_display = if receipt.expires == 0 {
        "Never".to_string()
    } else {
        receipt.expires.to_string()
    };
...
<div class="safe">This file contains no secret material (no mnemonic, no passphrase, no private key). It is safe to store in Google Drive, Dropbox, email, or print.</div>
...
<h3>Reproducible build (Nix)</h3>
<pre>nix build github:jpdarago/bip39key/{commit_short}
